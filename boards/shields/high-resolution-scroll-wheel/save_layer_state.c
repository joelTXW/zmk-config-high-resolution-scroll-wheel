...
static int sensitivity_settings_set(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg) {
    ...
    if (settings_name_steq(name, "layer", &next) && !next) {
        ...
        if (rc >= 0) {
            ...
            // Default layer is 0, so if we load something else, apply it now
...
static void save_layer_work_handler(struct k_work *work) {
    LOG_INF("Saving sensitivity layer to flash: %d", saved_layer);
...
static int save_layer_event_handler(const zmk_event_t *eh) {
    ...
    if (highest <= 3) {
        if (saved_layer != highest) {
            saved_layer = highest;
            // Delay save to prevent NVS flash wear on rapid taps
            k_work_reschedule(&save_layer_work, K_MSEC(CONFIG_ZMK_SETTINGS_SAVE_DEBOUNCE));
        }
    }
    return 0;
}
